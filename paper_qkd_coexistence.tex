...
Each edge $e \in E$ provides two types of capacit\begin{itemize}
...
\caption{Finite-key decoy-state BB84 SKR vs.\ fiber distance for 0–8 co-propagating classical channels at $-25$~dBm/channel.}
...
\caption{Total blocking rate vs.\ offered load for the four routing strategies.}
\label{fig:routing}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{results/key_blocking_rate_vs_load.png}}
\caption{Quantum key-blocking rate relative to Min-Hop vs.\ offered load for the four routing strategies.}
\label{fig:key_blocking}
\end{figure}

Fig.~\ref{fig:routing} shows the total blocking rate across 50--400 Erlangs; classical-bandwidth blocking is essentially zero in this regime, so all blocking is due to QKD key capacity. Fig.~\ref{fig:key_blocking} compares the four strategies' key-blocking rate relative to min-hop: the strategies differ by at most $\approx 1.2$~percentage points, with min-distance generally lowest at low-to-medium load. Fig.~\ref{fig:utilisation} shows the average classical utilisation, which stays below 20\% across all loads.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{results/classical_utilization_vs_load.png}}
\caption{Average classical-channel utilisation vs.\ offered load for the four routing strategies.}
...
\caption{DQN vs.\ first-fit wavelength assignment (30 evaluation episodes, 1000 steps each): blocking rate, aggregate SKR, and episode reward.}
\label{fig:rl_bars}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{results/eval_rl_curves.png}}
\caption{DQN training reward curve (10 masked evaluation episodes every 5K steps), with the first-fit baseline shown for reference.}
\label{fig:rl_curves}
\end{figure}
...
\caption{Joint routing + wavelength assignment: blocking rate (left) and aggregate SKR (right) for first-fit vs.\ DQN wavelength assignment under each routing strategy.}
\label{fig:joint_rwa}
\end{figure}
