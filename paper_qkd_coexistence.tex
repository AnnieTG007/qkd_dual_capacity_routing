...
\hline
Blocking Rate       & \textbf{20.88\%} & 23.64\% \\
Classical Util.     & \textbf{2.44\%}  & 2.37\%  \\
Agg.\ SKR (kbps)   & \textbf{1162.2}  & 1111.8  \\
Episode Reward      & \textbf{1126.0}  & 825.2   \\
\hline
\end{tabular}
\end{table}

The DQN achieves a \textbf{$-11.7\%$} reduction in blocking rate relative to first-fit (20.88\% vs.\ 23.64\%) and a \textbf{$+4.5\%$} gain in aggregate SKR (1{,}162~kbps vs.\ 1{,}112~kbps). Blocking reason analysis shows that mixed classical-and-key blocks account for $\approx77\%$ of all blocked requests in both policies, indicating that the bottleneck lies in the joint resource constraint rather than pure wavelength availability. The DQN's improvement stems from learning to avoid FWM-resonant wavelength placements that would degrade the quantum channel SKR, effectively translating physical-layer noise awareness into better long-term resource decisions.
...
The key findings are threefold. First, the simulation framework integrates a self-contained finite-key decoy-state BB84 model with discrete FWM and SpRS noise computation, enabling dynamic per-link key capacity evaluation that depends on instantaneous classical wavelength occupancy. Second, the dominant noise impairment under conservative launch power ($-25$~dBm/channel) and 50~GHz channel spacing is spontaneous Raman scattering; FWM is strongly phase-mismatch-suppressed at this spacing. Third, the DQN wavelength assignment agent outperforms first-fit on blocking rate ($-11.7\%$) and aggregate SKR ($+4.5\%$).

The dual-capacity-aware (DCA) routing strategy achieves a 5.7\% reduction in total blocking at medium loads (200~Erlangs) compared to minimum-hop routing, and the DQN wavelength agent further reduces blocking by 11.7\% and improves aggregate SKR by 4.5\% over first-fit. Blocking cause analysis shows that $\approx77\%$ of blocks arise from mixed classical-and-key capacity exhaustion, confirming that the fundamental bottleneck is the joint dual-capacity constraint rather than wavelength-only scarcity.
