...
\begin{abstract}
This paper studies joint routing and wavelength assignment (RWA) for quantum key distribution (QKD) coexisting with classical traffic in WDM optical networks. Four-wave mixing (FWM) and spontaneous Raman scattering (SpRS) from classical channels degrade per-link key capacity in a wavelength-dependent manner, creating a dual-constrained resource allocation problem. We propose a two-layer framework: four path-selection strategies with increasing QKD awareness at the routing layer, and a DQN agent at the wavelength assignment layer. Experiments on the NSFNET 14-node topology show that with 32 wavelength slots per edge, QKD key capacity is the sole bottleneck: classical blocking is negligible and min-distance routing reduces key-blocking by up to 1.2 percentage points over min-hop at low-to-medium load by naturally selecting shorter, lower-noise paths. The DQN wavelength assignment agent further reduces blocking by 9.3\% and improves aggregate secret key rate by 4.0\% over first-fit assignment.
\end{abstract}
...
Integrating QKD into existing WDM optical networks is economically attractive, as it reuses deployed fiber infrastructure instead of requiring dedicated dark fibers \cite{b2}. However, the extreme power disparity between classical (0--10~dBm) and quantum (single-photon-level) signals causes two dominant nonlinear impairments: four-wave mixing (FWM) and spontaneous Raman scattering (SpRS) \cite{b3}. Both effects depend on \emph{which wavelengths} carry classical traffic, so the secret key rate (SKR) available on each link varies with the current wavelength occupancy.

This creates a \textbf{dual-constrained RWA problem}: each link provides classical bandwidth (Gbps) and key capacity (kbps), where the latter changes dynamically with traffic state. Prior work either assumes fixed key capacities \cite{b4} or proposes static wavelength plans without dynamic traffic \cite{b5,b6}. Deep reinforcement learning (DRL) has shown promise for classical RWA \cite{b7} but has not been applied to QKD-aware coexistence networks.
...
\hline
Blocking Rate       & \textbf{25.91\%} & 28.56\% \\
Classical Util.     & \textbf{2.46\%}  & 2.36\%  \\
Agg.\ SKR (kbps)   & \textbf{1156.6}  & 1111.7  \\
Episode Reward      & \textbf{842.7}   & 562.4   \\
\hline
\end{tabular}
\end{table}

The DQN achieves a \textbf{$-9.3\%$} reduction in blocking rate relative to first-fit (25.91\% vs.\ 28.56\%) and a \textbf{$+4.0\%$} gain in aggregate SKR (1{,}157~kbps vs.\ 1{,}112~kbps). Blocking reason analysis shows that mixed classical-and-key blocks account for $\approx73\%$ of all blocked requests in both policies, indicating that the bottleneck lies in the joint resource constraint rather than pure wavelength availability. The DQN's improvement stems from learning to avoid FWM-resonant wavelength placements that would degrade the quantum channel SKR, effectively translating physical-layer noise awareness into better long-term resource decisions.
...
The key findings are threefold. First, the simulation framework integrates a self-contained finite-key decoy-state BB84 model with discrete FWM and SpRS noise computation, enabling dynamic per-link key capacity evaluation that depends on instantaneous classical wavelength occupancy. Second, the dominant noise impairment under conservative launch power ($-25$~dBm/channel) and 50~GHz channel spacing is spontaneous Raman scattering; FWM is strongly phase-mismatch-suppressed at this spacing. Third, the DQN wavelength assignment agent outperforms first-fit on blocking rate ($-9.3\%$) and aggregate SKR ($+4.0\%$).

Minimum-distance (MD) routing achieves up to a 1.2~percentage-point reduction in key-blocking at low-to-medium loads compared to minimum-hop routing, and the DQN wavelength agent further reduces blocking by 9.3\% and improves aggregate SKR by 4.0\% over first-fit. Blocking cause analysis shows that $\approx73\%$ of blocks arise from mixed classical-and-key capacity exhaustion, confirming that the fundamental bottleneck is the joint dual-capacity constraint rather than wavelength-only scarcity.
...
\begin{verbatim}
# 1. Routing simulation (4 strategies, load sweep 50-400 Erlang):
python run.py --qkd-capacity-mode actual_skr \
  --load-start 50 --load-end 400 --load-step 50 \
  --num-requests 5000 --seed 42

# 2. DQN training (500K-step budget, early-stopped
#    at ~140K steps, ~0.7 h on a single GPU):
python -m src.rl.train_dqn \
  --total-timesteps 500000 --learning-rate 3e-4 \
  --learning-starts 10000 --target-update-interval 5000 \
  --exploration-final-eps 0.05 --eval-freq 5000 \
  --n-eval-episodes 10 --max-steps 2000 --device cuda

# 3. Evaluation (DQN vs First-Fit, 30 episodes):
python -m src.rl.eval_rl \
  --n-episodes 30 --max-steps 1000 --seed 100
\end{verbatim}
